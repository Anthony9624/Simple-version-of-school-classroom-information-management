...
<body>
    ...
    <tr><td>
	    <center>
	    	<font size=5>教室借用记录</font>
		    <form id="sCourse" name="sCourse" method="post" action="<%=basePath%>admin/index.jsp?mainPage=opBorrow/lookBorrows&id=1">
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="详情" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/detailBorrow&page=lookBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:60px;height:30px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/delBorrow&page=lookBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:60px;height:30px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
	...
	if (getCurrentPage() > 1) {
	%>
	<a href ="<%=basePath%>admin/index.jsp?mainPage=opBorrow/lookBorrows&id=<%=getCurrentPage() - 1%>">上一页</a>
	<%
	}%>
	<%
	if (getCurrentPage() < getAllPages()) {
	%>
	<a href ="<%=basePath%>admin/index.jsp?mainPage=opBorrow/lookBorrows&id=<%=getCurrentPage() + 1%>">下一页</a>
	<%
	}%>
   	<%
  	dbCourse.disConnect();
