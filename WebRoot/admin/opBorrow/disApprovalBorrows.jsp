...
<body>
    ...
    <tr><td>
	    <center>
		    ...
		    <P>教室编号:<input type="text" name="classroomNum" size=15/>
		   	...
		   	<input type="submit" value="查找" name="submit" style="width:80px;height:30px"/>
		    </form>
	    </center>
	</td></tr>
	</table>
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="详情" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/detailBorrow&page=disApprovalBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:30px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="取消批准" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/disApprovalOk&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:70px;height:30px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/delBorrow&page=disApprovalBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:30px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
