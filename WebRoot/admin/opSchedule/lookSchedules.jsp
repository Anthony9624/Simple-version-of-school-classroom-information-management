...
<body>
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="修改" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opSchedule/alterSchedule&clno=<%=getClnoFromRS(i)%>&w=<%=getWeekdayFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:30px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opSchedule/delSchedule&clno=<%=getClnoFromRS(i)%>&w=<%=getWeekdayFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:30px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
