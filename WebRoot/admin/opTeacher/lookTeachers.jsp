...
<body>
	...
	public String getTnoFromRS(int index) {
		try {
			rs.absolute(index);
	...
	<%
		...
		if (strName != null && strName != "") {
			...
			if (strSearchType.equals("fuzzy")) {
				str += "where tname like '%"+strName+"%' ";
			} else {
				str += "where tname='"+strName+"' ";
			}
    ...
    <tr><td>
	    <center>
	    	<font size=5>教师信息</font>
		    <form id="sCourse" name="sCourse" method="post" action="<%=basePath%>admin/index.jsp?mainPage=opTeacher/lookTeachers&id=1">
		    <P>教师名称:<input type="text" name="courseName" size=15/>
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="修改" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opTeacher/alterTeacher&tno=<%=getTnoFromRS(i)%>')" style="width:50px;height:18px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opTeacher/delTeacher&tno=<%=getTnoFromRS(i)%>')" style="width:50px;height:18px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
	...
	if (getCurrentPage() > 1) {
	%>
	<a href ="<%=basePath%>admin/index.jsp?mainPage=opTeacher/lookTeachers&id=<%=getCurrentPage() - 1%>">上一页</a>
	<%
	}%>
	<%
	if (getCurrentPage() < getAllPages()) {
	%>
	<a href ="<%=basePath%>admin/index.jsp?mainPage=opTeacher/lookTeachers&id=<%=getCurrentPage() + 1%>">下一页</a>
	<%
	}%>
   	<%
  	dbCourse.disConnect();
