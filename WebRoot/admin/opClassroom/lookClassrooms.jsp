...
<body>
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="修改" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opClassroom/alterClassroom&clno=<%=getClnoFromRS(i)%>')" style="width:50px;height:30px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opClassroom/delClassroom&clno=<%=getClnoFromRS(i)%>')" style="width:50px;height:30px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
